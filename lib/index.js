...
process.version = process.version || '5.10.1';
...
(function() {
     ...
     *      - options (Object): options to be passed in to the method, see the documentation for the passed in method
     *      - callback (Function): function to call when the request is finished with an error as first argument and result data as second argument.
     *
     *  Follow all next links and returned the combined data of all pages
     */
    this.getAllPages = function (method, options, callback) {
        options.per_page = options.per_page || 100
        var result = []
        var cont = function (err, list) {
            if (err)
                return callback(err)

            result = result.concat(list)
            if (!this.hasNextPage(list)) {
                return callback(null, result)
            }
            this.getNextPage(list, cont)
        }.bind(this)
        method(options, cont)
    };
